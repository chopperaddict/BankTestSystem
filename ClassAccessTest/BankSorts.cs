		...
		//===================================================================
		{
			...
			{   // we have been asked for it in Descending order
				...
				foreach ( BankAccount Bk in descbankaccounts )
				{
					...
					if ( !BankAccount.BankDict.ContainsKey (Bk.BankAccountNumber) )
						BankAccount.BankDict.Add (Bk.BankAccountNumber, Bk);
		...
		{// Sort by A/C balance?
			...
			if ( dir )
			{
				...
				for ( int w = 0; w < cnt; w++ )
				{
					...
					if ( !BankAccount.BankDict.ContainsKey (BK[w].BankAccountNumber) )
						BankAccount.BankDict.Add (BK[w].BankAccountNumber, BK[w]);
				}
